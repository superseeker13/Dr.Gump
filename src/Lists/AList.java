 ...
 * @param <T>
 */
public class AList<T> implements ListWithListIterator<T>, Serializable {

    int numberOfEntries = 0;
    final transient int DEFAULT_CAPACITY = 25;
    final transient int MAX_CAPACITY = 1000;
    int capacity;
    T[] list;
    ...
    public AList(AList<AList<Character>> aList) {
        this.list = (T[]) new Object[aList.size()];
        capacity = aList.size();
        list = (T[]) aList.getListCharacterCopy();
        numberOfEntries = capacity;
    }
    ...
    public AList[] getListCharacterCopy() {
        AList[] copy = new AList[size()];
        for (int i = 0; i < size(); i++) {
            AList temp =  new AList(size());
            for(int j = 0; j < ((AList)list[i]).size(); j++){
                temp.add((Character)((AList)list[i]).get(j));
            }
            copy[i] = temp;
        }
        return copy;
